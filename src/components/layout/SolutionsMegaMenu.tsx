            ...
            <div className="grid grid-cols-2 gap-4">
                {solutions.map((item, index) => (
                    <Link
                        key={index}
                        href={item.href}
                        className="group/item flex items-start gap-4 p-4 rounded-2xl hover:bg-white/60 transition-all duration-200"
                    >
                        <div className={`w-12 h-12 rounded-xl ${item.color} flex items-center justify-center text-white shadow-lg shadow-black/5 group-hover/item:scale-110 transition-transform duration-300`}>
                            <item.icon size={24} />
                        </div>
                        <div>
                            <h3 className="font-bold text-gray-900 text-lg group-hover/item:text-brand-dark transition-colors flex items-center gap-2">
                                {item.title}
                                <ArrowRight className="w-4 h-4 opacity-0 -translate-x-2 group-hover/item:opacity-100 group-hover/item:translate-x-0 transition-all duration-300 text-gray-400" />
                            </h3>
