            ...
            >
                <div className={`mx-4 md:mx-auto max-w-7xl rounded-2xl transition-all duration-300 ${isScrolled ? 'glass-ios shadow-lg bg-white/80' : 'bg-transparent'}`}>
                    <div className="px-6 h-16 flex items-center justify-between">
                        ...
                        {/* Desktop Navigation */}
                        <nav className={`hidden md:flex items-center gap-8 font-medium text-sm ${isScrolled ? 'text-gray-600' : 'text-gray-200'}`}>
                            {['Home', 'Frota', 'Transportes', 'Blog', 'Contato'].map((item) => (
                                <Link
                                    key={item}
                        ...
                        {/* Mobile Toggle */}
                        <button
                            className={`md:hidden p-2 rounded-full ${isScrolled ? 'text-gray-900 bg-gray-100' : 'text-white bg-white/20 backdrop-blur'}`}
